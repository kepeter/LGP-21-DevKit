export const instructionDoc: Record<string, string> = {
    'A':
        '**ADD** — Add the contents of location m to the contents of the Accumulator. ' +
    ...
    'S':
        '**SUBTRACT** — Subtract the contents of location m from the contents of the Accumulator and retain the difference in the Accumulator. ' +
        'If a subtraction results in a number beyond the limits of the Accumulator, overflow will occur. ' +
        'Memory remains unaltered.',

    'M':
        '**MULTIPLY** — Multiply the contents of the Accumulator by the contents of location m, forming a 62-bit product ' +
    ...
    'D':
        '**DIVIDE** — Divide the number in the Accumulator by the number in location m, retaining the quotient, rounded to 30 bits, in the Accumulator. ' +
    ...
    'E':
        '**EXTRACT** — Where `1` bits are in location m, retain the value of the corresponding bit positions in the Accumulator; ' +
    ...
    'B':
        '**BRING** — Bring the contents of location m into the Accumulator, replacing its contents. Memory remains unchanged.',

    'H':
        '**HOLD** — Store the contents of the Accumulator into location m, without altering the contents of the Accumulator.',

    'C':
        '**CLEAR** — Store the contents of the Accumulator into memory location m; then clear the Accumulator to zero.',

    'R':
        '**SET RETURN ADDRESS** — In the address portion of location m, record the address which is 2 greater than ' +
        'the location of the R instruction being executed (i.e., the contents of the Counter Register plus 1).',

    'Y':
        '**STORE ADDRESS** — Replace the address portion of the word in location m with the address portion of the word in the Accumulator, ' +
        'leaving the rest of m and all of the Accumulator undisturbed.',

    'U':
        '**UNCONDITIONAL TRANSFER** — Replace the contents of the address portion of the Counter Register with m ' +
        'and get the next instruction from location m.',

    'T':
        '**CONDITIONAL TRANSFER** — If the contents of the Accumulator is negative (1 in the sign position), ' +
    ...
    '-T':
        '**TRANSFER CONTROL** — If the contents of the Accumulator is negative, or if the TC switch on the console is ON, ' +
        'replace the contents of the address portion of the Counter Register with m and get the next instruction from location m.',

    'Z':
        '**STOP / NO-OP / SENSE BRANCH SWITCHES** — Behaviour depends on the track portion of the address:\n\n' +
        '| Address | Action |\n' +
        '|---|---|\n' +
        '| `0000`, `0100` | **Halt** |\n' +
        '| `0200`, `0300` | **No-op** |\n' +
    ...
    '-Z':
        '**SENSE OVERFLOW AND TRANSFER** — If overflow is OFF (`0` in the sign position of the Counter Register), skip the next instruction in sequence. ' +
    ...
    'I':
        '**6-BIT SHIFT / INPUT** — Shift the contents of the Accumulator left 6 places, inserting zeros at the right. ' +
    ...
    '-I':
        '**4-BIT SHIFT / INPUT** — Shift the contents of the Accumulator left 4 places, inserting zeros at the right. ' +
    ...
    'P':
        '**6-BIT PRINT** — Transmit the character represented by bits 0 through 5 of the Accumulator to the output device. ' +
    ...
    '-P':
        '**4-BIT PRINT** — Combine `1` for channel 5 and `0` for channel 6 with bits 0 through 3 from the Accumulator, ' +
